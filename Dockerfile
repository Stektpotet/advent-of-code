...
RUN cd /tmp && wget https://ziglang.org/download/0.9.1/zig-linux-x86_64-0.9.1.tar.xz\
  && tar xf zig-linux-x86_64-0.9.1.tar.xz\
  && mkdir -p /bin/lib\
  && cp "zig-linux-x86_64-0.9.1/zig" /bin/\
  && cp -ru zig-linux-x86_64-0.9.1/lib/* /bin/lib/\
  && chmod ugo+x /bin/zig\
  && rm -rf /tmp/*
